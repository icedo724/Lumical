...
public class recal extends AppCompatActivity {
        ...
        dcal.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    index6 = index5 * (2.0477 * overload); // 스공/뎀퍼/최뎀 * (보공+뎀퍼) * 방무 * (최뎀+오마나)
                    ...
                    if (checkone > 900000000*1.05){
                        floor = 67;
                        층수.setText(floor + "층 이상(판정 불가)");
                    }
                    else if (checkone <= 900000000*1.05 && checkone > 810000000*1.05){
                        floor = 66;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 810000000*1.05 && checkone > 720000000*1.05){
                        floor = 65;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 720000000*1.05 && checkone > 6400000001.05){
                        floor = 64;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 640000000*1.05 && checkone > 571500000*1.05){
                        floor = 63;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 571500000*1.05 && checkone > 510000000*1.05){
                        floor = 62;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 510000000*1.05 && checkone > 450000000*1.05){
                        floor = 61;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 450000000*1.05 && checkone > 410000000*1.05) {
                        floor = 60;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 410000000*1.05 && checkone > 325000000*1.05){
                        floor = 59;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 325000000*1.05 && checkone > 280000000*1.05){
                        floor = 58;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 280000000*1.05 && checkone > 245000000*1.05){
                        floor = 57;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 245000000*1.05 && checkone > 212000000*1.05){
                        floor = 56;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 212000000*1.05 && checkone > 184000000*1.05){
                        floor = 55;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 184000000*1.05 && checkone > 160000000*1.05){
                        floor = 54;
                        층수.setText(floor + "층");
                    }
                    else if (checkone <= 160000000*1.05){
                        floor = 53;
                        층수.setText(floor + "층 이하(판정 불가)");
                    }
                    else {
                        Toast.makeText(recal.this, "오류 발생 : 다시 시도해주세요.", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    Toast.makeText(recal.this, "올바른 값을 입력해주세요", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    public void fromrecaltobuild(View view) {
        Intent intent = new Intent(getApplicationContext(), build.class);
        startActivity(intent);
    }
    public void fromrecaltomain(View view) {
